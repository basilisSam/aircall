...
import {generateGroupsByDate, getAllCalls, updateArchivedCalls, updateCall, updateCalls,} from "../../service/calls";
import {APP_KEY, AUTH_ENDPOINT, CHANNEL_NAME, CLUSTER, EVENT_NAME, TOAST_DEFAULT_CONFIG} from "../../constants";
    ...
    useEffect(() => {
        getAllCalls()
            .then((calls) => {
                ...
                setArchiveCalls(calls.nodes.filter((call) => call.is_archived === true));
                setIsLoading(false);
            })
            .catch((e) => {
                if (e.status === 401) {
    ...
    const handleUpdateCall = (id) => {
        updateCall(id)
            .then((updatedCall) => {
                ...
                toast.success("Call has been updated successfully!", TOAST_DEFAULT_CONFIG);
            });
    };
